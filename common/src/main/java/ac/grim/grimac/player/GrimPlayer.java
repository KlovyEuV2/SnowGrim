...
import ac.grim.grimac.utils.collisions.WebUtil;
...
public class GrimPlayer implements GrimUser {
    ...
    public long lastAttackedRealTick;
    public double lastWasClimbing = 0;
    public boolean attacking(int ticks) {return this.hitticks < ticks;}
    public int hitticks;
    ...
    public boolean wasTouchingLava, wasWasTouchingLava = false;
    ...
    public double lastDeltaX, lastDeltaZ, lastDeltaY, lastDeltaXZ;
    ...
    public static class RotData {
        ...
        public double lastDeltaXRot;
        public double lastDeltaYRot;
        public long lastHighRate;
        public long lastSmooth;
        public boolean cinematicRotation;
        public int isTotallyNotCinematic;
        public final List<Double> yawSamples = new ArrayList<>();
        public final List<Double> pitchSamples = new ArrayList<>();

        public RotData(double deltaYaw, double deltaPitch) {
            this.deltaYaw = deltaYaw;
            this.deltaPitch = deltaPitch;
            this.lastHighRate = 0L;
            this.lastSmooth = 0L;
            this.cinematicRotation = false;
    ...
    public GrimPlayer(@NotNull User user) {
        ...
        this.rotationData = new RotData(0, 0);
        rotatationUpdateData = new PlayerRotationData(this);
...
//        });
    ...
    public boolean isSprintSimulation() {
        ...
        double distXZ = GrimMath.distanceXZ(this.x, this.lastX, this.z, this.lastZ);
        ValuedAttribute speedAttribute = this.compensatedEntities.self.getAttribute(Attributes.MOVEMENT_SPEED).orElseThrow();
        final double defSpeed = (!this.onGround && !this.lastOnGround) ? speedAttribute.attribute().getDefaultValue() : speedAttribute.get() *7;
        double finalSpeed = defSpeed;
        double thresholdMultiplier = 0.256;
        boolean groundSpeedUpping = this.isSprinting && (this.onGround || this.lastOnGround);
//        if (groundSpeedUpping) finalSpeed *= 1.3;
        finalSpeed *= thresholdMultiplier;
        finalSpeed += 0.05;
        return distXZ > finalSpeed;
    }
    ...
    public void processCinematicRotation(double deltaYaw, double deltaPitch) {
        rotationData.deltaYaw = deltaYaw;
        rotationData.deltaPitch = deltaPitch;

        long now = System.currentTimeMillis();
        double differenceYaw = Math.abs(rotationData.deltaYaw - rotationData.lastDeltaXRot);
        double differencePitch = Math.abs(rotationData.deltaPitch - rotationData.lastDeltaYRot);
        double joltYaw = Math.abs(differenceYaw - rotationData.deltaYaw);
        double joltPitch = Math.abs(differencePitch - rotationData.deltaPitch);
        ...
        rotationData.yawSamples.add(rotationData.deltaYaw);
        rotationData.pitchSamples.add(rotationData.deltaPitch);
        ...
        rotationData.lastDeltaXRot = rotationData.deltaYaw;
        rotationData.lastDeltaYRot = rotationData.deltaPitch;
    }
